...
import logging
from IsoCARE.preprocessing.prepare import get_cubes_list,get_noise_level, prepare_first_iter_odd, prepare_first_iter_even
from IsoCARE.util.dict2attr import save_args_json,load_args_from_json
import numpy as np
import os
import sys
import shutil
from IsoCARE.util.metadata import MetaData
from IsoCARE.util.utils import mkfolder
...
def run(args):
    ...
    try:
        ...
        logging.info('\n######IsoCARE starts refining######\n')

        if args.continue_from is not None:
            logging.info('\n######IsoCARE Continues Refining######\n')
        ...
        if True:
            check_gpu(args)
            from IsoCARE.models.unet.predict import predict_odd, predict_even
            from IsoCARE.models.unet.train import prepare_first_model, train_data
        ...
        for num_iter in range(current_iter,args.iterations + 1):        
            ...
            #    np.random.setseed(seed)
            #    args.mrc_list_even = np.random.choice(args.all_mrc_list_even, size = int(args.select_subtomo_number), replace = False)
            #    args.mrc_list_odd = np.random.choice(args.all_mrc_list_odd, size = int(args.select_subtomo_number), replace = False)
            #else:
            args.mrc_list_even = args.all_mrc_list_even
            args.mrc_list_odd = args.all_mrc_list_odd
           ...
           #     return 1 / (1 + math.exp(-x))
            ...
            #        
            #else:
            #itercount = (0-1/(args.iter_count+1))*40
            #print('Chance to run IsoCARE:')
            #print(sigmoid(itercount))
            #for i in range(0,len(settings.mrc_list_odd)):
            #        if (np.random.rand() > 0.5):  
            #            if (np.random.rand() > sigmoid(itercount)):  
            #                mrc_list_x.append(args.mrc_list_odd[i])
            #                mrc_list_y.append(args.mrc_list_even[i])
            #            else:
            #                mrc_list_x.append(args.mrc_list_odd[i])
            #                mrc_list_y.append(args.mrc_list_odd[i])
            #        else:
            #            if (np.random.rand() > sigmoid(itercount)):
            #                mrc_list_y.append(args.mrc_list_odd[i])
            #                mrc_list_x.append(args.mrc_list_even[i])
            #            else:
            #                mrc_list_y.append(args.mrc_list_even[i])
            #                mrc_list_x.append(args.mrc_list_even[i])
            ...
            if args.pretrained_model is not None:
                ...
                logging.info("Start predicting even subtomograms!")
                predict_even(args)
                logging.info("Done predicting even subtomograms!")
                logging.info("Start predicting odd subtomograms!")
                predict_odd(args)
                logging.info("Done predicting odd subtomograms!")
                # if (args.iter_count % 2 == 0):
                #     logging.info("This Iteration trains odd -> even")
                # else:
                #     logging.info("This Iteration trains even -> odd")
                ...
                directory_path_even = 'results/even/'
                if not os.path.exists(directory_path_even):
                    os.makedirs(directory_path_even)
                directory_path_odd = 'results/odd/'
                if not os.path.exists(directory_path_odd):
                    os.makedirs(directory_path_odd)
                logging.info("Preparing first even data!")
                prepare_first_iter_even(args)
                logging.info("Done preparing first even data!")
                logging.info("Preparing first odd data!")
                prepare_first_iter_odd(args)
                logging.info("Done preparing first odd data!")
                # if (args.iter_count % 2 == 0):
                #     logging.info("This Iteration trains odd -> even")
                # else:
                #     logging.info("This Iteration trains even -> odd")
                ...
                ######## change this predict to predict odd and even each
                logging.info("Start predicting even subtomograms!")
                predict_even(args)
                logging.info("Done predicting even subtomograms!")
                logging.info("Start predicting odd subtomograms!")
            ...
            if num_iter>=args.noise_start_iter[0] and (not os.path.isdir(args.noise_dir) or len(os.listdir(args.noise_dir))< num_noise_volume ):
                from IsoCARE.util.noise_generator import make_noise_folder
...
def run_whole(args):
    ...
    '''
    md = MetaData()
    md.read(args.subtomo_star_even)
    ...
    if args.data_dir_odd is None:
        args.data_dir_odd = args.result_dir + '/odd'
    if args.data_dir_even is None:
        args.data_dir_even = args.result_dir + '/even'
    ...
    if len(md) <=0:
        logging.error("Subtomo even list is empty!")
        sys.exit(0)
    args.all_mrc_list_even = []
    for i,it in enumerate(md):
        if "rlnImageName" in md.getLabels():
            args.all_mrc_list_even.append(it.rlnImageName)
            

    md_odd = MetaData()
    md_odd.read(args.subtomo_star_odd)
    if len(md_odd) <=0:
        logging.error("Subtomo odd list is empty!")
        sys.exit(0)
    args.all_mrc_list_odd = []
    for i,it in enumerate(md_odd):
        if "rlnImageName" in md.getLabels():
            args.all_mrc_list_odd.append(it.rlnImageName)
    return args
