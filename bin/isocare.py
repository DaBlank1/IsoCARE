...
import numpy as np
from IsoCARE.util.dict2attr import Arg,check_parse,idx2list
...
class IsoCARE:
    ...
    def prepare_star(self,folder_name, output_star='tomograms.star',pixel_size = 10.0, defocus = 0.0, number_subtomos = 100):
        ...
        IsoCARE.py prepare_star folder_name [--output_star] [--pixel_size] [--defocus] [--number_subtomos]
    ...
    def prepare_subtomo_star(self, folder_name, output_star='subtomo.star', pixel_size: float=10.0, cube_size = None):
        ...
        This command is usually not necessary in the traditional workflow, because "IsoCARE.py extract" will generate this subtomo.star for you.\n
        IsoCARE.py prepare_subtomo_star folder_name [--output_star] [--cube_size]
    ...
    def deconv(self, star_file: str,
        ...
        IsoCARE.py deconv star_file [--deconv_folder] [--snrfalloff] [--deconvstrength] [--highpassnyquist] [--overlap_rate] [--ncpu] [--tomo_idx]
        ...
        """
        from IsoCARE.util.deconvolution import deconv_one
        np.random.seed(14)
        logging.basicConfig(format='%(asctime)s, %(levelname)-8s %(message)s',
        datefmt="%m-%d %H:%M:%S",level=logging.INFO,handlers=[logging.StreamHandler(sys.stdout)])
        logging.info('\n######IsoCARE starts ctf deconvolve######\n')
    ...
    def make_mask(self,star_file,
        ...
        IsoCARE.py make_mask star_file [--mask_folder] [--patch_size] [--density_percentage] [--std_percentage] [--use_deconv_tomo] [--tomo_idx]
        ...
        :param tomo_idx: (None) If this value is set, process only the tomograms listed in this index. e.g. 1,2,4 or 5-10,15,16
        """
        from IsoCARE.bin.make_mask import make_mask
        logging.basicConfig(format='%(asctime)s, %(levelname)-8s %(message)s',
        datefmt="%m-%d %H:%M:%S",level=logging.INFO,handlers=[logging.StreamHandler(sys.stdout)])
        logging.info('\n######IsoCARE starts making mask######\n')
        np.random.seed(14)
        try:
    ...
    def extract(self,
        ...
        IsoCARE.py extract star_file [--subtomo_folder] [--subtomo_star] [--cube_size] [--use_deconv_tomo] [--tomo_idx]
        ...
        logging.info("\n######IsoCARE starts extracting subtomograms######\n")
        ...
        try:
            ...
            from IsoCARE.preprocessing.prepare import extract_subtomos
            ...
            logging.info("\n######IsoCARE done extracting subtomograms######\n")
        except Exception:
            error_text = traceback.format_exc()
            f =open('log.txt','a+')
            f.write(error_text)
    ...
    def refine(self,
        #subtomo_star: str,
        subtomo_star_odd: str,      
        subtomo_star_even: str,       
        gpuID: str = None,
        iterations: int = None,
        data_dir_odd: str = None,
        data_dir_even: str = None,
        data_dir: str = None,
        ...
        noise_level:  tuple=(0.0,0.0),
        noise_start_iter: tuple=(50,55),
        noise_mode: str = None,
        noise_dir: str = None,
        ...
        IsoCARE.py refine subtomo_star [--iterations] [--gpuID] [--preprocessing_ncpus] [--batch_size] [--steps_per_epoch] [--noise_start_iter] [--noise_level]...
        ...
        ************************Network settings************************
        ...
        """
        from IsoCARE.bin.refine import run
        d = locals()
        d_args = Arg(d)
    ...
    def predict(self, star_file_odd: str, star_file_even: str, model: str, output_dir: str='./corrected_tomos', gpuID: str = None, cube_size:int=64,
    crop_size:int=96,use_deconv_tomo=True, batch_size:int=None,normalize_percentile: bool=True,log_level: str="info", tomo_idx=None):
        ...
        IsoCARE.py predict star_file model [--gpuID] [--output_dir] [--cube_size] [--crop_size] [--batch_size] [--tomo_idx]
        ...
        d = locals()
        d_args = Arg(d)
        from IsoCARE.bin.predict import predict
    ...
    def check(self):
        ...
        import PyQt5
        import tqdm
        print('IsoCARE --version 0.2 installed')

    def gui(self):
        """
        \nGraphic User Interface\n
        """
        import IsoCARE.gui.IsoCARE_star_app as app
        app.main()
