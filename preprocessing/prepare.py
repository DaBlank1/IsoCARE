...
import sys
import mrcfile
from IsoCARE.preprocessing.cubes import create_cube_seeds,crop_cubes,DataCubes
from IsoCARE.preprocessing.img_processing import normalize
from IsoCARE.preprocessing.simulate import apply_wedge1 as  apply_wedge, mw2d
from IsoCARE.preprocessing.simulate import apply_wedge_dcube
from multiprocessing import Pool
import numpy as np
from functools import partial
from IsoCARE.util.rotations import rotation_list
# from difflib import get_close_matches
from IsoCARE.util.metadata import MetaData, Item, Label
...
def extract_subtomos(settings):
    ...
    for it in md:
        if settings.tomo_idx is None or str(it.rlnIndex) in settings.tomo_idx:
            ...
            seeds=create_cube_seeds(orig_data, it.rlnNumberSubtomo, settings.crop_size,mask=mask_data,seed=14)
...
def get_cubes_one_odd(data_X, settings, start = 0, mask = None, add_noise = 0):
    '''
...
#         else:
#             from IsoCARE.util.noise_generator import make_noise_one
...
def get_cubes_one_even(data_Y, settings, start = 0, mask = None, add_noise = 0):
    with mrcfile.new('{}/train_y/y_{}.mrc'.format(settings.data_dir, start), overwrite=True) as output_mrc:
    ...
    #         output_mrc.set_data(data_Y.astype(np.float32))
    return 0
def get_cubes_odd(inp,settings):
    '''
    ...
    #### here we do try-except because the even/odd files can be in either even or odd directory, this way we can make sure that we find the correct file to prepare for training/testing
    # try:
    current_mrc = '{}/{}_iter{:0>2d}.mrc'.format(settings.data_dir_odd,root_name,settings.iter_count-1)
    # except FileNotFoundError:
    #     current_mrc = '{}/{}_odd_iter{:0>2d}.mrc'.format(settings.data_dir_even,root_name,settings.iter_count-1)
    ...
    for i in range(len(rotation_list)): 
        #if (settings.iter_count < 2):
        #    data_X = crop_to_size(rotated_data[i], settings.crop_size, settings.cube_size)
        #    get_cubes_one_odd(data_X, settings, start = start) 
...
def get_cubes_even(inp,settings):
    '''
    current iteration mrc(in the 'results') + infomation from orignal subtomo
    normalized predicted + normalized orig -> normalize
    rotate by rotation_list and feed to get_cubes_one
    ...
    #### here we do try-except because the even/odd files can be in either even or odd directory, this way we can make sure that we find the correct file to prepare for training/testing
    # try:
    current_mrc = '{}/{}_iter{:0>2d}.mrc'.format(settings.data_dir_even,root_name,settings.iter_count-1)
    # except FileNotFoundError:
    #     current_mrc = '{}/{}_even_iter{:0>2d}.mrc'.format(settings.data_dir_odd,root_name,settings.iter_count-1)
    with mrcfile.open(mrc) as mrcData:
        iw_data = mrcData.data.astype(np.float32)*-1
    iw_data = normalize(iw_data, percentile = settings.normalize_percentile)
    ...
    for i in range(len(rotation_list)): 
        data_Y = crop_to_size(rotated_data[i], settings.crop_size, settings.cube_size)
        get_cubes_one_even(data_Y, settings, start = start) 
        start += 1#settings.ncube
#     if (settings.iter_count % 2 == 0):
#         for i in range(len(rotation_list)): 
#            #data_X = crop_to_size(datax[i], settings.crop_size, settings.cube_size)
...
#     else:
#         for i in range(len(rotation_list)): 
#             data_Y = crop_to_size(datax[i], settings.crop_size, settings.cube_size)
#                # else:
#                #     data_Y = crop_to_size(rotated_data[i], settings.crop_size, settings.cube_size)
...
def get_cubes_list(settings):
    ...
    #dirs_tomake = ['train_xe','train_yo','train_xo','train_ye','test_xe','test_yo','test_xo','test_ye']
    ...
    # for i in range(0,len(settings.mrc_list_odd)):
    #         if (np.random.rand() > 0.5):   
    #             mrc_list_x.append(settings.mrc_list_odd[i])
    #             mrc_list_y.append(settings.mrc_list_even[i])
    #         else:
    #             mrc_list_y.append(settings.mrc_list_odd[i])
    #             mrc_list_x.append(settings.mrc_list_even[i])
    ...
    #        
    #else:
    #itercount = (0-1/(settings.iter_count+1))*50
    #print('Chance to run IsoCARE:')
    #print("Zero") #sigmoid(itercount))
    
    
    
    for i in range(0,len(settings.mrc_list_odd)):
        #if (np.random.rand() > sigmoid(itercount)):  
        mrc_list_x.append(settings.mrc_list_odd[i])
        mrc_list_y.append(settings.mrc_list_even[i])
        mrc_list_x1.append(settings.mrc_list_even[i])
        mrc_list_y1.append(settings.mrc_list_odd[i])
        #else:
        #    mrc_list_x.append(settings.mrc_list_odd[i])
        #    mrc_list_y.append(settings.mrc_list_odd[i])
        #    mrc_list_x1.append(settings.mrc_list_even[i])
        #    mrc_list_y1.append(settings.mrc_list_even[i])
            
    for i in range(0,len(settings.mrc_list_odd)):
        mrc_list_x.append(mrc_list_x1[i])
        mrc_list_y.append(mrc_list_y1[i])
#    for i in range(0,len(settings.mrc_list_odd)):
#            if (np.random.rand() > 0.5):  
#                if (np.random.rand() > sigmoid(itercount)):  
#                    mrc_list_x.append(settings.mrc_list_odd[i])
...
#                    mrc_list_y.append(settings.mrc_list_odd[i])
#                    mrc_list_x.append(settings.mrc_list_even[i])
#                    mrc_list_x1.append(settings.mrc_list_odd[i])
#                    mrc_list_y1.append(settings.mrc_list_even[i])
#                else:
#                    mrc_list_y.append(settings.mrc_list_even[i])
#                    mrc_list_x.append(settings.mrc_list_even[i])
...
#                    mrc_list_y1.append(settings.mrc_list_odd[i])
    ...
    logging.info("MRC_List_length: ")
    logging.info(len(mrc_list_x))
    ...
    ################### lorenz fragen
    ####hier erstellen wir die ODD X Daten
    # inp=[]
    # for i,mrc in enumerate(settings.mrc_list_odd):
    #     inp.append((mrc, i*len(rotation_list)))
        
    inp=[]
    for i,mrc in enumerate(mrc_list_x):
        inp.append((mrc, i*len(rotation_list)))
    
    # inp: list 0f (mrc_dir, index * rotation times)
    if settings.preprocessing_ncpus > 1:
        func = partial(get_cubes_odd, settings=settings)
        with Pool(settings.preprocessing_ncpus) as p:
            p.map(func,inp)
    else:
        for i in inp:
            logging.info("{}".format(i))
            get_cubes_odd(i, settings)

    ####hier erstellen wir die EVEN Y Daten
    # inp=[]
    # for i,mrc in enumerate(settings.mrc_list_even):
    #     inp.append((mrc, i*len(rotation_list)))
    
    inp=[]
    for i,mrc in enumerate(mrc_list_y):
        inp.append((mrc, i*len(rotation_list)))
    
    # inp: list 0f (mrc_dir, index * rotation times)

    if settings.preprocessing_ncpus > 1:
        func = partial(get_cubes_even, settings=settings)
        with Pool(settings.preprocessing_ncpus) as p:
            p.map(func,inp)
    else:
        for i in inp:
            logging.info("{}".format(i))
            get_cubes_even(i, settings)
...
def generate_first_iter_mrc_even(mrc,settings):
    '''
    ...
    with mrcfile.new('{}/{}_iter00.{}'.format(settings.data_dir_even,root_name, extension), overwrite=True) as output_mrc:
        output_mrc.set_data(-orig_data)

def generate_first_iter_mrc_odd(mrc,settings):
    '''
    Apply mw to the mrc and save as xx_iter00.xx
    '''
    root_name = mrc.split('/')[-1].split('.')[0]
    ...
    with mrcfile.new('{}/{}_iter00.{}'.format(settings.data_dir_odd,root_name, extension), overwrite=True) as output_mrc:
        output_mrc.set_data(-orig_data)

    #preparation files for the first iteration
def prepare_first_iter_even(settings):
    np.random.seed(14)
    if settings.preprocessing_ncpus >1:
        with Pool(settings.preprocessing_ncpus) as p:
            func = partial(generate_first_iter_mrc_even, settings=settings)
            p.map(func, settings.mrc_list_even)
    else:
        for i in settings.mrc_list_even:
            generate_first_iter_mrc_even(i,settings)
    return settings

def prepare_first_iter_odd(settings):
    if settings.preprocessing_ncpus >1:
        with Pool(settings.preprocessing_ncpus) as p:
            func = partial(generate_first_iter_mrc_odd, settings=settings)
            p.map(func, settings.mrc_list_odd)
    else:
        for i in settings.mrc_list_odd:
            generate_first_iter_mrc_odd(i,settings)
    return settings
