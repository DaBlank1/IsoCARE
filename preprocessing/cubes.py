...
from IsoCARE.preprocessing.simulate import apply_wedge_dcube as apply_wedge
from IsoCARE.util.noise_generator import make_noise_one
# from IsoCARE.preprocessing.simulate import apply_wedge

def create_cube_seeds(img3D,nCubesPerImg,cubeSideLen,mask=None,seed=None):
    if seed is not None:
        np.random.seed(seed)
...
# def create_cube_seeds_extract(valid_inds,sample_inds):
#     rand_inds = [v[sample_inds] for v in valid_inds]
#     return (rand_inds[0],rand_inds[1], rand_inds[2])
